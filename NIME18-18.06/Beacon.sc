...
Beacon {
	...
	classvar <>server, <>buf;
	var directory, filenames, bufferonsets, bufferpingpong, bufferrecording, bufferbeattrack, window, vol;
	var iter, j, k;
	...
	*new { |numnodes = 2, offset = 0, audioinbus = 0|
		...
		directory = "/Users/annaxambo/Documents/__Postdoc\ Projects__/Ongoing/NIME\ performance/Piece\ NIME18/sounds_NIME18";

		server.waitForBoot {

	    bufferonsets = Buffer.alloc(server, 512);
		bufferpingpong = Buffer.alloc(server, 44100 * 2, numspeakers);
		bufferrecording = Buffer.read(server, directory +/+ "Lavelier-recording-mono.wav");
		bufferbeattrack = Buffer.alloc(server, 1024, 1); //  for sampling rates 44100 and 48000

		window = Window.new("Server Levels");
		ServerMeterView.new(server, window, 0@0, 2, 2);
		window.front; // show the window
		vol = server.volume;
		vol.gui;

		SynthDef('kickdrum', { | amp = 1, freq = 90, da = 0, bypassfx = 0, rate = 0.1 | // freq: 60-80; rate: 0.1-8
				var oscout, osc1, env1;
				var sig, chain, onsets;
				var out, effect;
				if (inputchannel == -1, {
					sig = PlayBuf.ar(1, bufferrecording, BufRateScale.kr(bufferrecording) * rate, loop: 1);
				},
				{
...
/*				if (bypassfx == 0, {
			...
			SynthDef('beattrkdef', { | out = 0, amp = 0.7, rate = 0.05 |
				...
				if (inputchannel == -1, {
					sig = PlayBuf.ar(1, bufferrecording, BufRateScale.kr(bufferrecording), loop: 1);
				},
				{
					sig = SoundIn.ar(inputchannel);
				}
				...
				beep = PinkNoise.ar(Decay.kr(trackb, rate));

				// Stereo case
				out = 0;
				oscout = Pan2.ar(beep) * amp;
/*				if (bypassfx == 0, {
					Out.ar(60, oscout);
				},
				{
		...
		SynthDef('audioin', { | amp = 0.7, feedback = 0.5, gate = 1 |
				...
				left = sig;
				right = sig;
				effect = PingPong.ar(bufferpingpong.bufnum, [ sig, sig ], 0.4, feedback, 0);
				out = 0;
...
/*				if (bypassfx == 0, {
			...
			SynthDef(\beacon_PlayBuf_HPF, { | amp = 1, rate = 1, freq = 300, bufnum = 0, gate = 1, bypassfx = 0, outbus = 23 |
				var oscout, loop, da;
				var sig, env, out;
				 loop = 1;
				da = 2;
...
/*				if (bypassfx == 0, {
			...
			SynthDef(\beacon_PlayBuf_random_speaker, { | amp = 1, rate = 1, gate = 1, bufnum = 0, freq = 440, trigger = 1, freqonsets = 0.2 |
				...
				if (inputchannel == -1, {
					in = PlayBuf.ar(1, bufferrecording, BufRateScale.kr(bufferrecording), loop: 1);
				},
				{
...
/*					//some MIRLC!
			...
			SynthDef(\beacon_PlayBuf_Saw_MIRLC, { | bus = 51, amp = 1, bufnum = 0, rate = 1, gate = 1, modrate = 100, freq = 440, freqonsets = 0.2 |
...
/*					//some MIRLC!
		...
		filenames = Dictionary.new;
			filenames = [
				...
				"spinning.wav",
				"female-voice-reversed.wav",
				"The-Crossley-Gas-Engine-turning-down.wav",
				"Lavelier-recording-mono.wav",
				"Die-Sinking-Workshop.wav"
			];
		...
		//Server.default.makeGui; // for manual recording!!!
